...
import useLikeQuestion from "@/src/queries/question/useLike";
...
const Like = () => {
    ...
    useEffect(() => {
        question._id && question?.likes.includes(user?._id!)
            ? setIsLiked(true)
            : setIsLiked(false);
    }, [user?._id, question.likes]);

    useEffect(() => {
        setLikesNumber(question.likes.length);
    }, [user?._id, question.likes]);

    const like = useLikeQuestion(question._id, _id!);
    ...
    return (
        <>
            <div className="flex items-center gap-1">
                <button onClick={handleLikeClick} disabled={like.isPending}>
                    {isLiked ? (
                        <svg
                ...
                <p className="mt-1 text-[.9rem] font-shabt text-neutral-300 tracking-[1px]">
                    {convertEngToPer(likesNumber)}
                </p>
            </div>
